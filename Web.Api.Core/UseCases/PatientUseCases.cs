    ...
    public sealed class PatientUseCases : IPatientUseCases
    {
        ...
        public async Task<bool> Handle(GetDetailsRequest request, IOutputPort<GetDetailsResponse> outputPort)
        {
            ...
            /*if(request.LableName == "DrCall")
                ...
                getDetailsResponse = new GetDetailsResponse(await _patientRepository.GetPatientDetails(request.CompanyId, request.PatientId, request.LableName, request.ScheduledFromDate, request.ScheduledToDate), true, "Data Fetched Successfully");
